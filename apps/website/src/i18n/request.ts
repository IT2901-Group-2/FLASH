 ...
 * based on request.
 *
 * It uses the `getRequestConfig` function from `next-intl/server` to load
 * the appropriate messages for the requested locale.
 *
 * The `hasLocale` function is used to check if the requested locale is
 * supported, and if not, it falls back to the default locale defined in
